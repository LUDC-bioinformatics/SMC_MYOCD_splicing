...
- `results/reports/WebGestaltR/` - report describing the analysis, figures and tables.
...
Then the counts were normilized to TPM and tested for differential transcript usage with
[SUPPA2](https://github.com/comprna/SUPPA#differential-transcript-usage).
The code is added to `code/Snakefile`.

The results are processed and visualized in R:


```bash
R -e 'rmarkdown::render("code/SUPPA.Rmd", output_dir="results/reports/")'
```

Results:

- `results/tables/SUPPA/SUPPA_all.csv` - SUPPA all results.

- `results/tables/SUPPA/SUPPA_FDR_0.05.pdf` - SUPPA significant results (FDR=0.05).

- `results/tables/SUPPA/SUPPA_FDR_0.05_PSI_0.1.pdf` - SUPPA significant results (FDR=0.05)
   with the PSI > 10%.
   
- `results/figures/SUPPA/` - plots for SUPPA significant results (FDR=0.05).
...
```bash
R -e 'rmarkdown::render("code/splicing_gene_overlap.Rmd", output_dir="results/reports/")'
```

Results:

   - `results/tables/splicing_overlap_gene/rMATS_DEXSeq_SUPPA_gene_overlap.csv` - genes that are significant in all three analyses.
   
   - `results/tables/splicing_overlap_gene/rMATS_DEXSeq_SUPPA_atleast2genes_overlap.csv` - genes that are significant in at
      least two analyses.
   
   - `results/figures/rmats_dexseq_suppa_gene_overlap.pdf` - Venn Diagram of overlap between the three analyses.

In addition, I also check if there are exact exons matches in the results.


```bash
R -e 'rmarkdown::render("code/splicing_exon_overlap.Rmd", output_dir="results/reports/")'
```

Results:

   - `results/tables/splicing_overlap_exon/` - results for DEXSeq overlap with rMATS and SUPPA based on exon overlaps.
   
*Note, only the comparison DEXSeq bins to rMATS exons is reliable here. SUPPA is transcript based, so it cannot be directly compared to the exon-based results., but I included it for additional information.*
