...
interesting_genes_rows <- grep(paste(interesting_genes,collapse="|"), d_sing_annot$symbol)
interesting_genes_sign <- unique(d_sing_annot$symbol[interesting_genes_rows])
interesting_genes_sign
```
...
```{r}
clust <- read.table('intermediate/suppa/salmon_TPM_isoform.clustvec',
                   col.names = c('id', 'cluster',	'CMV',	'MYO'),
                   sep = "\t", header = F)
...
library(reshape2)
library(ggplot2)
#levels(clust$cluster)

# plot
for (i in c(0:2)){
  #i <- 0
  d <- clust[clust$cluster==i,]
  d_melt <- melt(d)
  gd <- ggplot(d_melt) +
          geom_point(aes(x = variable,
                         ...
                         color = variable,
                         shape = variable),
                       position=position_jitter(w=0.01,h=0), show.legend = FALSE) +
          ylab('PSI') +
          ggtitle('Isoform cluster') +
          geom_line(aes(x = variable,
                        y = value,
                        group = id,
                        color = id), size=0.2, alpha=0.3, show.legend = FALSE) +
          theme_bw() +
