...
annotAll <- queryMany(unique(gene_renamed),
                      scopes='ensembl.gene',
                      fields='symbol,name,summary', # pathway.kegg.id,go
                      return.as='DataFrame',
                      species='human',
                      returnall=FALSE)
# add full gene names
...
annot <- as.data.frame(annotAll[-duplNoSumNoFound,
                                c('gene', 'query', 'symbol', 'name', 'summary')])
...
```{r}
filePath <- 'results/tables/diff_expression_genes/'
dir.create(filePath, showWarnings = FALSE)
# all results
...
```{bash, include=FALSE}
cd results/tables/diff_expression_genes/
for i in DESeq_*.csv;
  do
    soffice --headless \
